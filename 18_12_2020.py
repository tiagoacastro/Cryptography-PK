...
def sha512(M):
    ...
    while i < len(M):
        W = []
        for j in range(16):
            W.append(parseUint64(M[(math.floor(i/128)*128 + j*8):(math.floor(i/128)*128 + j*8 + 8)]))
        for j in range(16, 80):
            W.append((W[j-16]+W[j-7]+s(W[j-15], 1, 8, 7)+s(W[j-2], 19, 61, 6)) & 0xFFFFFFFFFFFFFFFF)
        for j in range(8):
            tmp[j] = H[j]
        for j in range(80):
            t1 = (K[j] + W[j] + S(tmp[4], 14, 18, 41) + Ch(tmp[4], tmp[5], tmp[6]) + tmp[7]) & 0xFFFFFFFFFFFFFFFF
            t2 = (Maj(tmp[0], tmp[1], tmp[2]) + S(tmp[0], 28, 34, 39)) & 0xFFFFFFFFFFFFFFFF
...
print(hex(sha512(bytearray(input("Input something\n").encode()))))